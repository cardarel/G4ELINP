...
void G4ELIMED_Run::RecordEvent(const G4Event* event)
{
  ...
  if ( fCollID_dose < 0 ) {
   fCollID_dose 
     = G4SDManager::GetSDMpointer()->GetCollectionID("multisd/dose");
   G4cout << " fCollID_patient: " << G4SDManager::GetSDMpointer()->GetCollectionID("multisd/dose") << G4endl;   
   G4cout << " fCollID_patient: " << G4SDManager::GetSDMpointer()->GetCollectionID("det/collection") << G4endl;   
